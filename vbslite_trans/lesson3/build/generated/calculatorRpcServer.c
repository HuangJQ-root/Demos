...
__attribute__((weak)) int32_t MVBS_calculator_add_svc(MVBS_calculator_add_in *req_type, MVBS_calculator_add_out *res_type)
{
	UNUSED_ARG(req_type);
	UNUSED_ARG(res_type);
...
static int32_t MVBS_calculator_add_handle(struct rpc_connection *conn,
					 struct mvbs_cdr *cdr,
					 struct rpc_srv_handler *handler,
					 struct rpc_header *header)
{
	...
	int32_t (*svc_cb)(MVBS_calculator_add_in *req_type, MVBS_calculator_add_out *res_type);

	/* step1: get current offset */
	offset = mcdr_buffer_length(cdr);

	/* step2: deserialize Param[IN] */
	if (MVBS_calculator_add_deserialize_input_params(cdr, &req_type) == false)
		goto fail;
	...
	if (MVBS_calculator_add_serialize_output_params(cdr, &res_type) == false)
			goto fail;
